...
func (m TokenModel) New(db *gorm.DB, userID int64, ttl time.Duration, scope string) (*Token, error) {
	token, err := GenerateToken(userID, ttl, scope)
	if err != nil {
		return nil, err
	}

	err = m.Insert(db, token)
	return token, err
}
...
func (m TokenModel) Insert(db *gorm.DB, token *Token) error {
...
func (m TokenModel) DeleteAllForUser(db *gorm.DB, scope string, userID int64) error {
	result := db.Where("scope = ? AND user_id = ?", scope, userID).Delete(&Token{})

	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {
		return errors.New("no tokens deleted")
	}
	return nil
}
